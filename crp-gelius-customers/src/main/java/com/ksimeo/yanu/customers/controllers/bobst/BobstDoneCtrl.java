...
/**
 * @author Ksimeo. Created on 01.09.2016 at 20:20 for "crp-gelius" project.
 * @version 1.0
 * @since 1.0
 */
@WebServlet(urlPatterns = "/partbobstdone.do")
public class BobstDoneCtrl extends HttpServlet {
    @Autowired
    private PlansService planServ;

    private static final Logger log = Logger.getLogger(BobstDoneCtrl.class);
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (id != null && !(id.equals("")) && q != null && !(q.equals(""))) {
            ...
            int quantDone = planServ.getPlan(planId).getBobstDone();
            quant = quantDone + quant;
            if (quant < planQuant) {
                planServ.getPlan(planId).setBobstDone(quant);
                resp.sendRedirect("/bobst.do");
            } else if (quant == planQuant) {
                resp.sendRedirect("/allbobstdone.do?id=" + planId);
            } else {
